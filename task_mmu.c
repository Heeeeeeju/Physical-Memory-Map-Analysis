...
static ssize_t ospagemap_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	...
	const int INFO_SIZE = 500;
	char *pagemap_buf = kmalloc(BUF_SIZE, GFP_KERNEL);
	int cur = 0, bytes = 0, num_of_process = 0;
	struct task_struct *p = NULL;
	struct task_info *info = kmalloc(INFO_SIZE, GFP_KERNEL);
	...
	for_each_process(p) {
		...
		(info + num_of_process)->pid = p->pid;
		sprintf((info + num_of_process)->pname, "%s", p->comm);
	...
	for (i = 0; i < num_of_process; i++) {
		...
		pid_struct = find_get_pid((info + i)->pid);
